...
public class LatinIME extends InputMethodService implements OnKeyboardActionListener, UiListener, OnSharedPreferenceChangeListener {
    ...
    public static final String BANGLA_UNICODE_FONT = "BengaliOTS.ttf";
    ...
    private MyDictionary mMyDictionary;
    ...
    public RidmikParser parser = new RidmikParser();
    ...
    public void onCreate() {
        ...
        this.learnWordAsTyping = prefs.getBoolean("learn_word_as_typing", true);
        ...
        FONT_SETTINGS = Integer.valueOf(prefs.getString("pref_font_settings", "3")).intValue();
    ...
    private void initSuggest(String locale) {
        ...
        this.mSuggest = new Suggest((Context) this, getDictionary(orig));
        updateAutoTextEnabled(saveLocale);
        if (this.mContactsDictionary == null) {
            this.mContactsDictionary = new ContactsDictionary(this, 4);
        }
        if (this.mMyDictionary != null) {
            this.mMyDictionary.close();
        }
        if (!(langNow == 1 || langNow == 3)) {
            bangla = false;
        }
        this.mMyDictionary = new MyDictionary(this, 2, bangla);
        this.mSuggest.setContactsDictionary(this.mContactsDictionary);
        this.mSuggest.setMyDictionary(this.mMyDictionary);
        ...
        if (this.mPredicting) {
            ...
            if (this.mComposing.length() > 0) {
                if (inputConnection != null) {
                    if (langNow == 1) {
                        inputConnection.commitText(this.parser.toBangla(this.mComposing.toString()), 1);
                    } else {
                        inputConnection.commitText(this.mComposing.toString(), 1);
                    }
                }
                if (langNow == 1) {
                    this.mCommittedLength = this.parser.toBangla(this.mComposing.toString()).length();
                } else {
        ...
        if (ic != null) {
            ...
            if (this.mShiftKeyState.isMomentary()) {
                ...
                if (length > 0) {
                    ...
                    if (langNow == 1) {
                        ic.setComposingText(this.parser.toBangla(this.mComposing.toString()), 1);
                    } else {
    ...
    private void handleVoiceResults() {
        ...
        if (preferCapitalization() || (this.mKeyboardSwitcher.isAlphabetMode() && this.mKeyboardSwitcher.getInputView().isShifted())) {
    ...
    private void showSuggestions(List<CharSequence> stringList, CharSequence typedWord, boolean typedWordValid, boolean correctionAvailable) {
        ...
        if (stringList.size() <= 0) {
            this.mBestWord = null;
        } else if (!correctionAvailable || typedWordValid || stringList.size() <= 1) {
    ...
    private void checkAddToDictionary(CharSequence suggestion, int frequencyDelta, boolean addToBigramDictionary) {
        if (!this.mPasswordText && suggestion != null && suggestion.length() >= 1) {
            if (!(this.mSuggest.isValidWord(suggestion.toString()) || this.mSuggest.isValidWord(suggestion.toString().toLowerCase()) || this.mMyDictionary.isValidWord(suggestion) || !this.learnWordAsTyping)) {
                this.mMyDictionary.addWord(suggestion.toString(), FREQUENCY_FOR_MYDIC);
            }
            if (this.mCorrectionMode != 2 && this.mCorrectionMode != 3) {
            }
        }
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if (PREF_SELECTED_LANGUAGES.equals(key)) {
            ...
            toggleLanguage(true, true);
        } else if ("pref_font_settings".equals(key)) {
            FONT_SETTINGS = Integer.valueOf(sharedPreferences.getString("pref_font_settings", "3")).intValue();
        }
        if (needReloadView) {
            this.mKeyboardSwitcher.makeKeyboards(true);
        }
    ...
    private void loadSettings() {
        ...
        if (sp.getBoolean(PREF_AUTO_CAP, false) && langNow == 2) {
            z = true;
        } else {
            z = false;
        }
